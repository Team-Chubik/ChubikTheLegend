...
# Načtení jednotlivých barev materiálů
...
# Získání jedinečného identifikátoru
var id = self.get_instance_id()

# Nastavení výběru na "false"
var selected = false

# funkce "_ready" => jestliže se entita načetla
func _ready():
	$Circle.set_surface_material(0,green)
	$Circle.set_surface_material(1,white)
	$Cylinder.set_surface_material(0,blue)

# funkce "_proces" => každý game tick zkontrolování jestli má být active
func _process(_delta):
	if(global.activeID != id):
		$Circle.set_surface_material(1,white)
		selected = false

# Jestliže na bylo kliknuto na dílek levým tlačítkem, změna stavu active [true/false]
